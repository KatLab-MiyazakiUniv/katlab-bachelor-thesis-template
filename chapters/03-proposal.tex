\chapter{提案手法}\label{cha:Proposal}

本章では、第1章で述べた課題を解決するための、LLMを用いたシステム仕様書章立て生成手法について提案する。

\section{提案手法の利用プロセス}
\label{sec:process}

本提案手法の利用プロセスは、以下の2ステップから構成される。

\begin{enumerate}
    \item \textbf{システム情報の入力}:
    ユーザー（仕様書作成者）は、本研究で作成したプロンプトの「開発するシステム情報」欄に、対象システムの概要を記述する。また必要に応じて「システム仕様書の定義」の微調整を行う。
...
本節では、上記プロセスで使用するプロンプトの具体的な構成について述べる。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|l|p{8cm}|l|} % 幅を調整
    ...
    \hline
    出力指示 & 論理的整合性の確保、機械可読性の担保 & CoT, Template Pattern \\
    \hline
  \end{tabular}
\end{table}
...
「専門アシスタント」というペルソナを与えることで、LLMの出力モードを専門文書作成に適したものに切り替える。
...
実務で使用されている標準的な章立てを「参考テンプレート」として提示している。
これは、第\ref{subsec:zero_few_shot}節で触れた\textbf{One-shot Prompting}の一種（構造の例示）と捉えることができる。
...
\textbf{課題：}
詳細なシステム情報がないまま章立てを行わせると、どのシステムにも当てはまるような具体的でない記述や、事実に基づかない内容（ハルシネーション）が生成される恐れがある。
...
また、テンプレートパターンにより出力形式を統一することで、可読性を最大化し、人間による検証（Human-in-the-loop）を効率的かつ確実に行えるようにすることを目的としている。

次章以降では、本章で作成したプロンプトを用いて実際に仕様書章立てを生成し、その網羅性や妥当性について評価を行う。
