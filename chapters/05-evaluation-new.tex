...
実験では、実際のシステム開発を想定した題材を用い、提案するプロンプトを用いた手法とその他の手法で作成した章立ての品質を、
...
\begin{itemize}
    ...
    \item \textbf{手法C（提案するプロンプト）}: 被験者はLLM(GPT-5.2)を使用し、本研究で作成したプロンプトを用いて章立てを作成する。
\end{itemize}
...
\begin{table}[tp]
  ...
  \begin{tabular}{|c|l|l|}
    ...
    \hline\hline
    A & 手法A（手動） & 手法C（提案するプロンプト） \\
    \hline
    B & 手法C（提案するプロンプト） & 手法A（手動） \\
    \hline
    C & 手法B（LLMのみ） & 手法C（提案するプロンプト） \\
    \hline
    D & 手法C（提案するプロンプト） & 手法B（LLMのみ） \\
    \hline
  \end{tabular}
\end{table}
...
BMP画像分割アプリにおいては、手法Cにより作成された章立ての平均点が最も高いという結果になった。
一方で、クラウド型電子契約サービスにおいては、手法Bにより作成された章立ての平均点が最も高いという結果になった。
...
2つのシステムに共通して、手法Bは、網羅性と独自性が高く、
手法Cは、可読性、拡張性、構造的整合性が高いという結果になった。
...
\subsubsection{被験者Bによる手法C（提案するプロンプト）}
%評価結果の表(行が評価項目、列が評価者)
%章立て
%傾向の分析
...
\begin{table}[tp]
  \caption{手法C（提案するプロンプト）の評価者別詳細評価結果}
...
\subsubsection{被験者Dによる手法C（提案するプロンプト）}
%評価結果の表(行が評価項目、列が評価者)
%章立て
%傾向の分析
...
\subsubsection{被験者Aによる手法C（提案するプロンプト）}
%評価結果の表(行が評価項目、列が評価者)
%章立て
%傾向の分析
...
\subsubsection{被験者Cによる手法C（提案するプロンプト）}
%評価結果の表(行が評価項目、列が評価者)
%章立て
%傾向の分析
...
%     \item \textbf{提案するプロンプトの柔軟性}: 手法Cで作成した2つの章立ては、章の順序や項目の配置に若干の違いがあるものの、いずれも高い評価を得た。これは、テンプレートプロンプトが一定の柔軟性を持ちながらも、品質を保証できることを示している。
% \end{itemize}
