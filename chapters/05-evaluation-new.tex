\chapter{評価実験と考察}\label{cha:Evaluation}

本研究で提案するプロンプトの有用性を検証するため、実験を行った。本実験の被験者は、システム仕様書作成経験のない学生4人である。
実験では、実際のシステム開発を想定した題材を用い、本プロンプトを用いた手法とその他の手法で作成した章立ての品質を、
株式会社スカイコム\cite{skycom}にて5年以上のシステム仕様書作成経験を持つ専門家5人が評価した。
...
\begin{itemize}
    ...
    \item \textbf{手法B（LLMのみ）}: 被験者はLLM(GPT-5.2)を使用するが、本研究のプロンプトは使用せず、自由なプロンプトで章立てを作成する。
    \item \textbf{手法C（本プロンプト）}: 被験者はLLM(GPT-5.2)を使用し、本研究で作成したプロンプトを用いて章立てを作成する。
\end{itemize}

手法Aと手法Bにおいて、被験者には、対象とするシステムの情報と、図\ref{fig:base_outline}に示す章立てのテンプレートを与えた。
...
\begin{table}[tp]
  ...
  \begin{tabular}{|c|l|l|}
    ...
    B & 手法C（本プロンプト） & 手法A（手動） \\
    \hline
    C & 手法B（LLMのみ） & 手法C（本プロンプト） \\
    \hline
    D & 手法C（本プロンプト） & 手法B（LLMのみ） \\
    \hline
  \end{tabular}
\end{table}
...
\subsection{評価方法}
作成した章立ては、株式会社スカイコムにて5年以上のシステム仕様書作成経験を持つ専門家によって評価された。
評価は以下の9項目について行い、それぞれ5段階のリッカート尺度\cite{likert}(5:非常に良い, 4:良い, 3:普通, 2:悪い, 1:非常に悪い)で採点された。
...
表\ref{tb:cloud}に、クラウド型電子契約サービスにおける評価結果を示す。表\ref{tb:cloud}内の数値は、評価者5人の平均点である。
...
BMP画像分割アプリにおいては、手法Cにより作成された章立ての平均点が最も高いという結果になった。一方で、クラウド型電子契約サービスにおいては、手法Bにより作成された章立ての平均点が最も高いという結果になった。
...
図\ref{fig:cloudchart}にクラウド型電子契約サービスの評価結果のレーダーチャートを示す。

2つのシステムに共通して、手法Bは、網羅性と独自性が高く、手法Cは、命名の適切さ、可読性、適合性が高いという結果になった。



\begin{figure}[tp]
  \centering
  \includegraphics[width=0.8\textwidth]{images/BMPレーダーチャート.png}
...
\begin{figure}[tp]
  \centering
  \includegraphics[width=0.8\textwidth]{images/クラウドレーダーチャート.png}
...
本節では、実験において作成されたBMP画像分割アプリの章立てについて述べる。
\subsubsection{手法A（手動作成）}
%評価結果の表(行が評価項目、列が評価者)
%章立て
%傾向の分析

図\ref{fig:A_chapter_structure_of_bmp_app}に手法Aにより作成されたBMP画像分割アプリの章立てを示す。
また、表\ref{tb:detailed_evaluation_results_A}に手法Aにより作成されたBMP画像分割アプリシステム仕様書の章立ての評価結果を示す。
...
\subsubsection{手法B（LLMのみ）}
%評価結果の表(行が評価項目、列が評価者)
%章立て
%傾向の分析
図\ref{fig:B_chapter_structure_of_bmp_app}に手法Bにより作成されたBMP画像分割アプリの章立てを示す。
また、表\ref{tb:detailed_evaluation_results_B}に手法Bにより作成されたBMP画像分割アプリシステム仕様書の章立ての評価結果を示す。
...
\subsubsection{被験者Bによる手法C（本プロンプト）}
%評価結果の表(行が評価項目、列が評価者)
%章立て
%傾向の分析

図\ref{fig:C_chapter_structure_of_bmp_app_1}に被験者Bが手法Cで作成したBMP画像分割アプリの章立てを示す。
また、表\ref{tb:detailed_evaluation_results_C}に被験者Bが手法Cで作成したBMP画像分割アプリシステム仕様書の章立ての評価結果を示す。
...
\begin{table}[tp]
  \caption{手法C（本プロンプト）の評価者別詳細評価結果}
...
\subsubsection{被験者Dによる手法C（本プロンプト）}
%評価結果の表(行が評価項目、列が評価者)
%章立て
%傾向の分析

図\ref{fig:C_chapter_structure_of_bmp_app_2}に被験者Dが手法Cで作成したBMP画像分割アプリの章立てを示す。
また、表\ref{tb:detailed_evaluation_results_C2}に被験者Dが手法Cで作成したBMP画像分割アプリシステム仕様書の章立ての評価結果を示す。
...
図\ref{fig:A_chapter_structure_of_cloud_contract_system}に、手法Aで作成されたクラウド型電子契約サービスの章立てを示す。
また、表\ref{tb:cloud_detailed_evaluation_results_A}に、手法Aで作成されたクラウド型電子契約サービスシステム仕様書の章立ての評価結果を示す。
...
図\ref{fig:B_chapter_structure_of_cloud_contract_system}に、手法Bで作成されたクラウド型電子契約サービスの章立てを示す。
また、表\ref{tb:cloud_detailed_evaluation_results_B}に、手法Bで作成されたクラウド型電子契約サービスシステム仕様書の章立ての評価結果を示す。
...
\subsubsection{被験者Aによる手法C（本プロンプト）}
%評価結果の表(行が評価項目、列が評価者)
%章立て
%傾向の分析

図\ref{fig:C_chapter_structure_of_cloud_contract_system_1}に、被験者Aが手法Cで作成したクラウド型電子契約サービスの章立てを示す。
また、表\ref{tb:cloud_detailed_evaluation_results_C}に、被験者Aが手法Cで作成したクラウド型電子契約サービスシステム仕様書の章立ての評価結果を示す。
...
\end{multicols}
\end{tcolorbox}
\caption{被験者Aが手法Cで作成したクラウド型電子契約システムの章立て}
...
\subsubsection{被験者Cによる手法C（本プロンプト）}
%評価結果の表(行が評価項目、列が評価者)
%章立て
%傾向の分析

図\ref{fig:C_chapter_structure_of_cloud_contract_system_2}に、被験者Cが手法Cで作成したクラウド型電子契約サービスの章立てを示す。
また、表\ref{tb:cloud_detailed_evaluation_results_C2}に、被験者Cが手法Cで作成したクラウド型電子契約サービスシステム仕様書の章立ての評価結果を示す。
...
\end{multicols}
\end{tcolorbox}
\caption{被験者Cが手法Cで作成したクラウド型電子契約システムの章立て}
...
% 上記の表より、手法C（本プロンプト）がすべての評価項目において最も高い平均スコアを記録していることが分かる。
% 特に「構造的整合性」では全評価者が5点を付けており、本プロンプトの有効性が明確に示されている。
...
% 表\ref{tb:evaluation_result_bmp}より、本プロンプトを用いた手法（手法C）がすべての評価項目において最も高いスコアを記録した。
% 特に「網羅性」（4.8）、「可読性」（4.6）、「適合性」（4.7）、「構造的整合性」（4.9）において顕著な高評価を得ており、本プロンプトの有効性が確認出来た。
% 手法Aは「論理性」（4.0）において比較的高いスコアを示したが、「詳細度」（3.0）や「拡張性」（3.0）では低い評価となった。
% 手法Bは全体的に中程度の評価であり、特に「詳細度」（2.5）と「構造的整合性」（3.2）において改善の余地が見られた。
...
% また、詳細度については、項レベルまで細分化されているものの、内容の粒度が不均一であり、改善の余地がある。

% \clearpage
% \begin{minipage}{\textwidth}
% \paragraph{手法C（本プロンプト）による章立て}
% 図\ref{fig:C_chapter_structure_of_bmp_app_1}と図\ref{fig:C_chapter_structure_of_bmp_app_2}に、手法Cで作成した2つの章立てを示す。
...
% \begin{itemize}
%     \item \textbf{本プロンプトの優位性}: 本プロンプトを用いた手法（手法C）は、すべての評価項目において最も高いスコアを記録し、特に「網羅性」「適合性」「構造的整合性」において顕著な優位性を示した。これは、テンプレートプロンプトが「ベース章立て」と「出力指示」を明示的に提供することで、体系的かつ一貫性のある章立てを生成できることを示している。
    
%     \item \textbf{手動作成の課題}: 手法Aは、経験に基づいた論理的な構成を示したものの、詳細度や拡張性において課題が見られた。特に、階層構造の不整合（第3章の節番号）は、手動作成における見落としの可能性を示唆している。
    
%     \item \textbf{LLMのみの使用の限界}: 手法Bは、手法Aよりも詳細な章立てを生成したが、章の論理的な関係性や粒度の均一性において改善の余地があった。これは、明確な指示がない場合、LLMが生成する章立ての品質にばらつきが生じることを示している。
    
%     \item \textbf{本プロンプトの再現性}: 手法Cで作成された2つの章立ては、異なる被験者によって作成されたにもかかわらず、高い類似性と一貫性を示した。これは、テンプレートプロンプトが再現性の高い章立て生成を可能にすることを示している。
% \end{itemize}
% \end{minipage}
...
%   \begin{tabular}{|l|c|c|c|}
%     \hline
%     \textbf{評価項目} & \textbf{手法A} & \textbf{手法B} & \textbf{手法C（本プロンプト）} \\
%     \hline\hline
...
% 表\ref{tb:evaluation_result_cloud}より、BMP画像分割アプリと同様に、本プロンプトを用いた手法（手法C）がすべての評価項目において最も高いスコアを記録した。
% 特に「網羅性」（4.8）、「可読性」（4.6）、「適合性」（4.7）、「構造的整合性」（4.9）において顕著な高評価を得ており、システムの複雑さに関わらず本プロンプトの有効性が確認された。
% 手法Aは「論理性」（4.0）において比較的高いスコアを示したが、BMP画像分割アプリと同様に「詳細度」（3.0）や「拡張性」（3.0）では低い評価となった。
% 手法Bは、特に「論理性」（2.5）と「構造的整合性」（2.8）において低い評価を受けており、複雑なシステムにおいてLLMのみでの章立て作成の難しさが示された。
...
% \begin{itemize}
%     \item \textbf{複雑なシステムにおける本プロンプトの有効性}: BMP画像分割アプリと同様に、本プロンプトを用いた手法（手法C）はすべての評価項目において最も高いスコアを記録した。特に、クラウド型電子契約サービスのような複雑なシステムにおいても、本プロンプトが高品質な章立てを生成できることを確認した。
...
%     \item \textbf{ドメイン知識の重要性}: 手法Aは、BMP画像分割アプリと同様の構造を採用したが、クラウド型電子契約サービス特有の「画面仕様」や「外部インターフェース」などの項目が不足していた。これは、手動作成においてドメイン知識が重要であることを示している。一方、本プロンプトを用いた手法は、テンプレートプロンプトによってドメイン知識を補完し、高品質な章立てを生成できることを確認した。
    
%     \item \textbf{本プロンプトの柔軟性}: 手法Cで作成した2つの章立ては、章の順序や項目の配置に若干の違いがあるものの、いずれも高い評価を得た。これは、テンプレートプロンプトが一定の柔軟性を持ちながらも、品質を保証できることを示している。
% \end{itemize}
...
% \subsubsection{本プロンプトの有効性}
% 実験の結果、本プロンプトを用いた手法（手法C）は、すべての評価項目において手法Aおよび手法Bを上回るスコアを記録した。
% 特に「網羅性」（4.8）、「可読性」（4.6）、「適合性」（4.7）、「構造的整合性」（4.9）において顕著な高さを示しており、これはプロンプトで与えた「ベース章立て」と「出力指示」の有効性を示唆している。

% 本プロンプトの優位性は、以下の要因によるものと考えられる：
...
% 手法A（手動作成）は、「論理性」において比較的高いスコアを示したが、「詳細度」や「拡張性」では低い評価となった。
% これは、経験に基づいた論理的な構成は可能であるものの、詳細な階層構造の設計や将来の拡張を考慮した設計には課題があることを示している。

% 手法B（LLMのみ）は、全体的に中程度の評価であり、特に「詳細度」と「構造的整合性」において改善の余地が見られた。
