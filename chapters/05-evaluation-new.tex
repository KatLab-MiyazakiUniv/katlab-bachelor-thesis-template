...
実験では、実際のシステム開発を想定した題材を用い、提案するプロンプトを用いた手法とその他の手法で、被験者が章立てを作成し、その章立ての品質を、
...
\begin{itemize}
    \item \textbf{手法A(手動作成)}: 被験者はLLMを用いず、章立てを作成する。また、被験者には、対象とするシステムの情報(\ref{sec:target_system}節参照)と、ベースの章立て(図\ref{fig:base_outline}参照)を与える。
    \item \textbf{手法B(自由プロンプト)}: 被験者はLLM(GPT-5.2)を使用して、章立てを作成する。この際、提案するプロンプトは使用せず、被験者自身による自由なプロンプトをLLM(GPT-5.2)に与える。また、被験者には、対象とするシステムの情報(\ref{sec:target_system}節参照)と、ベースの章立て(図\ref{fig:base_outline}参照)を与える。
    \item \textbf{手法C(提案するプロンプト)}: 被験者はLLM(GPT-5.2)を使用し、提案するプロンプトを用いて章立てを作成する。
\end{itemize}
...
\begin{table}[tp]
  ...
  \begin{tabular}{|c|l|l|}
    ...
    \hline
    C & 手法B(自由プロンプト) & 手法C(提案するプロンプト) \\
    \hline
    D & 手法C(提案するプロンプト) & 手法B(自由プロンプト) \\
    \hline
  \end{tabular}
\end{table}

\subsection{評価方法}
作成した章立ては、株式会社スカイコムにて5年以上のシステム仕様書作成経験を持つ専門家が評価した。
...
そのため、すべての章立てに対して個別のコメントが得られているわけではないが、得られたコメントについては\ref{sec:evaluation_results}節において示す。
...
図\ref{fig:A_chapter_structure_of_bmp_app}に手法Aを用いて作成したBMP画像分割アプリの章立てを示す。
また、表\ref{tb:detailed_evaluation_results_A}に手法Aを用いて作成したBMP画像分割アプリシステム仕様書の章立ての評価結果を示す。
また、以下に評価者のコメントを示す。
...
%章立て
%傾向の分析
図\ref{fig:B_chapter_structure_of_bmp_app}に手法Bを用いて作成したBMP画像分割アプリの章立てを示す。
また、表\ref{tb:detailed_evaluation_results_B}に手法Bを用いて作成したBMP画像分割アプリシステム仕様書の章立ての評価結果を示す。
また、以下に評価者のコメントを示す。
...
\begin{table}[tp]
  \caption{手法B(自由プロンプト)の評価者別詳細評価結果}
...
図\ref{fig:C_chapter_structure_of_bmp_app_1}に被験者Bが手法Cを用いて作成したBMP画像分割アプリの章立てを示す。
また、表\ref{tb:detailed_evaluation_results_C}に被験者Bが手法Cを用いて作成したBMP画像分割アプリシステム仕様書の章立ての評価結果を示す。
また、以下に評価者のコメントを示す。
...
\end{multicols}
\end{tcolorbox}
\caption{被験者Bが手法C(提案するプロンプト)で作成した章立て}
\label{fig:C_chapter_structure_of_bmp_app_1}
\end{figure}
\FloatBarrier

\begin{table}[tp]
  \caption{被験者Bによる手法C(提案するプロンプト)章立ての評価者別詳細評価結果}
...
図\ref{fig:C_chapter_structure_of_bmp_app_2}に被験者Dが手法Cを用いて作成したBMP画像分割アプリの章立てを示す。
また、表\ref{tb:detailed_evaluation_results_C2}に被験者Dが手法Cを用いて作成したBMP画像分割アプリシステム仕様書の章立ての評価結果を示す。
また、以下に評価者のコメントを示す。
...
\end{multicols}
\end{tcolorbox}
\caption{被験者Dが手法C(提案するプロンプト)で作成した章立て}
\label{fig:C_chapter_structure_of_bmp_app_2}
\end{figure}
\FloatBarrier

\begin{table}[tp]
  \caption{被験者Dによる手法C(提案するプロンプト)章立ての評価者別詳細評価結果}
...
また、表\ref{tb:cloud_detailed_evaluation_results_A}に、手法Aを用いて作成したクラウド型電子契約サービスシステム仕様書の章立ての評価結果を示す。
また、以下に評価者のコメントを示す。
...
\begin{table}[tp]
  \caption{手法A(手動作成)章立ての評価者別詳細評価結果}
...
また、表\ref{tb:cloud_detailed_evaluation_results_B}に、手法Bを用いて作成したクラウド型電子契約サービスシステム仕様書の章立ての評価結果を示す。
また、以下に評価者のコメントを示す。
...
\begin{table}[tp]
  \caption{手法B(自由プロンプト)章立ての評価者別詳細評価結果}
...
また、表\ref{tb:cloud_detailed_evaluation_results_C}に、被験者Aが手法Cを用いて作成したクラウド型電子契約サービスシステム仕様書の章立ての評価結果を示す。
また、以下に評価者のコメントを示す。
...
\end{multicols}
\end{tcolorbox}
\caption{被験者Aが手法Cを用いて作成したクラウド型電子契約システムの章立て}
...
\begin{table}[tp]
  \caption{被験者Aによる手法C(提案するプロンプト)章立ての評価者別詳細評価結果}
...
また、表\ref{tb:cloud_detailed_evaluation_results_C2}に、被験者Cが手法Cを用いて作成したクラウド型電子契約サービスシステム仕様書の章立ての評価結果を示す。
また、以下に評価者のコメントを示す。
...
\end{multicols}
\end{tcolorbox}
\caption{被験者Cが手法Cを用いて作成したクラウド型電子契約システムの章立て}
...
\begin{table}[tp]
  \caption{被験者Cによる手法C(提案するプロンプト)章立ての評価者別詳細評価結果}
