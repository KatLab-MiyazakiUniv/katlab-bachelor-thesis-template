...
\begin{itemize}
    \item \textbf{手法A(手動作成)}: 被験者はLLMを用いず、章立てを作成する。この際、被験者には、対象システムの情報(\ref{sec:target_system}節参照)と、ベースの章立て(図\ref{fig:base_outline}参照)を与える。
    \item \textbf{手法B(自由プロンプト)}: 被験者はLLM(GPT-5.2)を使用して、章立てを作成する。この際、被験者には、対象とするシステムの情報(\ref{sec:target_system}節参照)と、ベースの章立て(図\ref{fig:base_outline}参照)を与える。ただし、提案するプロンプトは与えず、被験者自身による自由なプロンプトをLLM(GPT-5.2)に与える。
